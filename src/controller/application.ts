...
export const getExecutionByLoggedInUserId = async (ctx: Context, next: () => void) => {
    const status: string | undefined = ctx.request.query.status === 'undefined' ? undefined : ctx.request.query.status as string;
    const type: string = ctx.request.query.type as string;
    const user: any = ctx.state.user;
...
export const getExecutionInProcessLoggedInUserId = async (ctx: Context, next: () => void) => {
    const status: string | undefined = ctx.request.query.status as string;
    const user: any = ctx.state.user;
...
export const getExecutionInProcessLoggedInUserIdByQuery = async (ctx: Context, next: () => void) => {
    ...
    const type: string = ctx.request.query.type as string;
    const user: any = ctx.state.user;
    const applicationId: string = ctx.request.query.applicationId as string;
    const startDate: string = ctx.request.query.startDate as string;
    const endDate: string = ctx.request.query.endDate as string;
...
export const getExecutionParticipatedQuery = async (ctx: Context, next: () => Promise<void>) => {
    const user: any = ctx.state.user;
    const searchText: string = ctx.request.query.searchText ? ctx.request.query.searchText.toString() : '';
    const startDate: string = ctx.request.query.startDate ? ctx.request.query.startDate.toString() : '';
    const endDate: string = ctx.request.query.endDate ? ctx.request.query.endDate.toString() : '';
...
export const getInProgressExecutions = async (ctx: Context, next: () => void) => {
    const user: any = ctx.state.user;
    const applicationId: string = ctx.params.applicationId as string;
    const startDate: string = ctx.request.query.startDate as string;
    const endDate: string = ctx.request.query.endDate as string;
...
export const getWithdrawExecutions = async (ctx: Context, next: () => void) => {
    const loggedInUser: string = ctx.state.user.userId; // Assuming userId is always available in ctx.state.user
    const payload: IGetWithdrawRequest = {
        applicationId: ctx.query.applicationId as string, // Ensure applicationId is of type string
...
export const getExecutionParticipatedUsers = async (ctx: Context, next: () => void) => {
    const loggedInUser: string = ctx.state.user.userId;
    const executionId: string = ctx.params.executionId as string;
...
export const reassignWorkflow = async (ctx: Context, next: () => void) => {
    const payload: IReassignExecutionRequest = {
        ...
        userId: ctx.request.body.userId as string, // Ensure userId is of type string
        applicationId: ctx.request.body.appId as string, // Ensure appId is of type string
        workflowId: ctx.request.body.workflowId as string,
    };
...
export const withdraw = async (ctx: Context, next: () => void) => {
    const executionId: string = ctx.params.executionId as string;
    const executionWorkflowId: string = ctx.params.executionWorkflowId as string;
    const userId: string = ctx.state.user.userId; // Assuming userId is always available in ctx.state.user
