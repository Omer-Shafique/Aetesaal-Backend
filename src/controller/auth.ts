...
export const login = async (ctx: Context, next: () => void) => {
  ...
  const appVersion = appVersionHeader ? parseFloat(Array.isArray(appVersionHeader) ? appVersionHeader[0] : appVersionHeader) : undefined;

  if (!platform || appVersion === undefined) {
    throw boom.forbidden('Newer version of the application is available. Please contact the administrator.');
